...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 0 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 3 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\encryption-events](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\encryption-events)
...
graph LR
  ...
  encryption_events["encryption_events"]:::center
```
...
- entity_pk (key)
- id (key)
- local_key_version (key)

## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package encryption-events -Apply
