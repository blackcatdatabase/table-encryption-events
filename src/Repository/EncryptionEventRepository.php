...
use BlackCat\Database\Packages\EncryptionEvents\Dto\EncryptionEventDto as Dto;
use BlackCat\Database\Packages\EncryptionEvents\Mapper\EncryptionEventDtoMapper as RowMapper;
...
final class EncryptionEventRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EncryptionEvents\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EncryptionEvents\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionEvents\Dto\EncryptionEventDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionEvents\Dto\EncryptionEventDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EncryptionEvents\Mapper\EncryptionEventDtoMapper::fromRow($row);
    }
