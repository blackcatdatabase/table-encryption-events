 ...
 */
final class EncryptionEventsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EncryptionEventRepository $encryptionEventRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'encryption_events' => $this->encryptionEventRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\EncryptionEvents\ModuleException("Record {$idStr} not found for locking.");
            }
